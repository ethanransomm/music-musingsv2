<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    ...
    <div class="flex flex-col items-center justify-center text-center mb-16">
        <div class="w-32 h-32 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white text-4xl font-bold shadow-lg mb-6">
    ...
    <div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6">
            @forelse($artist->albums as $album)
                <a href="{{ route('album.show', $album->id) }}" class="group block">
                    

                    <div class="aspect-square bg-gray-100 rounded-lg overflow-hidden shadow-sm group-hover:shadow-md transition mb-3 relative">
                        @if($album->cover_url)
                            <img src="{{ $album->cover_url }}" class="w-full h-full object-cover group-hover:opacity-90 transition">
                        @else
                    ...
                    <h3 class="font-bold text-gray-900 leading-tight group-hover:text-indigo-600 transition">
                        {{ $album->title }}
                    </h3>
            ...
            @empty
                <p class="col-span-full text-center text-gray-400 italic">No albums found for this artist.</p>
            @endforelse
        </div>
    </div>
</div>