    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 space-y-24">
        ...
        <div class="text-center space-y-6 py-24">
            <h1 class="text-7xl md:text-8xl font-black text-white tracking-tight mb-6 leading-none">
                Music <span class="bg-gradient-to-r from-green-400 via-green-500 to-green-600 bg-clip-text text-transparent">Musings</span>
            </h1>
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-8">
                @foreach($featuredAlbums as $album)
                    <a href="{{ route('album.show', $album->id) }}"
                        ...
                        <div class="aspect-square w-full bg-gray-900 rounded-xl overflow-hidden mb-5 relative shadow-xl ring-1 ring-gray-700/50 group-hover:ring-green-500/50 transition-all duration-300">
                            @if($album->cover_url)
                                <img src="{{ $album->cover_url }}" alt="{{ $album->title }}"
                                    class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500">
                            @else
                                <div class="absolute inset-0 flex items-center justify-center text-gray-700 group-hover:text-gray-600 transition-colors duration-300">
                        ...
                        <h3 class="font-bold text-white text-lg truncate group-hover:text-green-400 transition-colors duration-300 mb-1">{{ $album->title }}</h3>
                        <p class="text-sm text-gray-400 truncate font-light">{{ $album->artist->artistName ?? 'Unknown' }}</p>
                    </a>
                @endforeach
            </div>
        </section>
        ...
        <section class="space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                @foreach($recentReviews as $review)
                    <div class="group bg-gray-800/50 backdrop-blur-sm p-8 rounded-2xl border border-gray-700/50 hover:border-green-500/50 transition-all duration-300 flex flex-col h-full transform hover:-translate-y-1 hover:shadow-2xl hover:shadow-green-500/10">

                        <div class="flex items-center space-x-4 mb-6">
                            <a href="{{ route('profile.show', $review->user) }}" class="flex-shrink-0 transform transition-transform duration-300 hover:scale-110">
                                ...
                                @else
                                    <div class="w-12 h-12 rounded-full bg-gradient-to-br from-green-400 to-blue-500 flex items-center justify-center text-black font-bold text-lg border-2 border-gray-700 group-hover:border-green-500 transition-colors duration-300 shadow-lg">
                        ...
                        <div class="flex-1 space-y-4">
                            ...
                            <p class="text-gray-300 text-sm line-clamp-3 leading-relaxed font-light italic">"{{ $review->comment }}"</p>
                        </div>
        ...
        <section class="space-y-8">
            ...
            <div class="grid grid-cols-3 md:grid-cols-6 gap-8">
                @foreach($featuredArtists as $artist)
                    <a href="{{ route('artist.show', $artist->id) }}" class="group text-center transform transition-all duration-300 hover:-translate-y-3">
                        <div class="aspect-square rounded-full bg-gray-800/50 backdrop-blur-sm border-4 border-gray-700 group-hover:border-green-500 transition-all duration-300 overflow-hidden mb-4 flex items-center justify-center shadow-xl group-hover:shadow-2xl group-hover:shadow-green-500/30 ring-4 ring-transparent group-hover:ring-green-500/20">
                            @if($artist->image_url)
                                <img src="{{ $artist->image_url }}" 
                                     ...
                                     class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500">
                            @else
                                <span class="text-3xl font-bold text-gray-400 group-hover:text-green-400 transition-colors duration-300">
                                    {{ substr($artist->artistName, 0, 1) }}
                                </span>
                            @endif
                        </div>
                        <p class="text-sm text-gray-300 font-semibold truncate group-hover:text-green-400 transition-colors duration-300">
                            {{ $artist->artistName }}
                        </p>
                    </a>
                @endforeach
