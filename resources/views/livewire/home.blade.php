<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-16">
    ...
    <div class="text-center space-y-4 py-16">
        ...
        <div class="pt-6">
            <a href="{{ route('albums.index') }}"
                class="px-8 py-3 bg-green-500 text-black font-bold rounded-full hover:bg-green-400 transition transform hover:scale-105 shadow-lg inline-block">
                Dive In
            </a>
        </div>
    </div>
    ...
    <section>
        <div class="flex items-center justify-between mb-6">
            ...
            <a href="{{ route('albums.index') }}"
                class="text-sm font-bold text-gray-400 hover:text-white uppercase tracking-wider transition">View
                All</a>
        </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
            @foreach($featuredAlbums as $album)
                <a href="{{ route('album.show', $album->id) }}"
                    class="group block bg-gray-800 rounded-lg p-4 hover:bg-gray-700 transition duration-300">
        ...
        <div class="grid grid-cols-3 md:grid-cols-6 gap-4">
            @foreach($featuredArtists as $artist)
                <a href="{{ route('artist.show', $artist->id) }}" class="group text-center">
                    <div
                        class="aspect-square rounded-full bg-gray-800 border-2 border-gray-700 group-hover:border-green-500 transition overflow-hidden mb-2 flex items-center justify-center">
